...
curl -X POST http://localhost:8000/api/v1/workflows/{workflow_id}/approve
...
curl -X POST http://localhost:8000/api/v1/workflows/{workflow_id}/reject \
  -H "Content-Type: application/json" \
...
curl -X POST http://localhost:8000/api/v1/workflows/non-existent/approve
# Expected: 404 WorkflowNotFoundError

# Try to approve already-completed workflow
curl -X POST http://localhost:8000/api/v1/workflows/{completed_id}/approve
...
curl http://localhost:8000/api/v1/workflows/{workflow_id}/events | jq '.[] | select(.event_type | contains("STAGE"))'
...
# Approve A
curl -X POST http://localhost:8000/api/v1/workflows/{workflow_id_a}/approve

# Check B is still blocked
curl http://localhost:8000/api/v1/workflows/{workflow_id_b}
# Expected: {"workflow_status": "blocked"}
```
