...
class TestSettingsRepository:
    ...
    async def test_get_server_settings_raises_if_not_initialized(self, repo: SettingsRepository):
    ...
    async def test_update_server_settings_invalid_field(self, repo: SettingsRepository):
    ...
    async def test_update_server_settings_empty_dict(self, repo: SettingsRepository):
        """Verify update_server_settings with empty dict returns current settings."""
        await repo.ensure_defaults()
        settings = await repo.update_server_settings({})
        assert settings.log_retention_days == 30

    async def test_update_server_settings_updates_timestamp(self, repo: SettingsRepository):
    ...
    async def test_update_all_settings_fields(self, repo: SettingsRepository):
