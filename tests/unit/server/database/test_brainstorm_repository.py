...
class TestRepositoryNoIsSystem(TestBrainstormRepository):
    """Tests verifying is_system is removed from repository."""

    async def test_get_messages_no_include_system_param(
        self, repository: BrainstormRepository
    ) -> None:
        """get_messages should not have include_system parameter."""
        import inspect

        sig = inspect.signature(repository.get_messages)
        assert "include_system" not in sig.parameters
